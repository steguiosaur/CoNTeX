# Tasks

A list of tasks that should be done on the project.

**vault.php**:

- [x] fix CSS for profile banner
- [x] Create new vault function
- [x] Read database for created vaults
- [ ] Update or rename vault name
- [ ] Delete Vault and its contents

**editor.php unfixed features**:

- [ ] Enable parsing for UML diagrams
- [ ] Syntax highlighting
- [x] Auto pair parenthesis
- [ ] Fix regex based parser to having an AST before rendering
- [ ] Synchonized the scroll directly to particular line
- [ ] Implement file tree for all notes
- [ ] Undo redo the written text
- [ ] Create file on particular vault
- [ ] Read from database the contents of vault to be placed on file tree
- [ ] Update or rename file on particular vault
- [ ] Delete file on particular vault
- [ ] Replace text on textarea when selected file is selected on file tree
- [ ] Auto save on server the contents of textarea on some interval or when exit
- [ ] Auto fix merge conflict for collaborative editing

**login.php**:

- [x] remember me cookies
- [ ] forgot password page
- [ ] login through username or email account

**README.md**:

- [ ] Steps to execute on current device
- [ ] Clarify the tutorial
- [ ] License

**footer.php**:

- [ ] Group members
- [ ] Github code link
- [ ] License
- [x] Copyright

**index.php**:

- [x] Create images for the project feature section
- [x] Navbar display current session username and profile picture
- [x] If no session, navbar show login and signup button (optional)
